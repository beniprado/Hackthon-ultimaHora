      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link href="/aulas" className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors">
      ...
      <form onSubmit={handleSaveConfig} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-[#e0f7fb] dark:bg-[#132234] border border-[#cbd5e1] dark:border-[#1e293b] rounded-xl p-5 shadow-lg backdrop-blur-md space-y-4">
            ...
            <p className="text-[11px] text-slate-600 dark:text-slate-400 pt-2 flex items-center gap-1.5">
          ...
          <div className="bg-cyan-50 dark:bg-cyan-950/40 border border-cyan-200 dark:border-cyan-800/60 rounded-xl p-5 shadow-lg">
            ...
            <p className="text-xs text-slate-800 dark:text-zinc-200 leading-relaxed font-medium">
              <strong className="text-[#0e7c93] dark:text-cyan-400">{selectedApps.length} aplicativo(s) liberado(s)</strong> para uso pedagógico.
            </p>
            <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-1">
              Turma: {selectedTurma?.nome || "1º A"} • {selectedDisciplina} ({selectedSala})
            </p>
            <div className="mt-3 pt-3 border-t border-cyan-200 dark:border-cyan-800/40 flex items-center gap-1.5 text-[10px] text-slate-600 dark:text-slate-400">
        ...
        <div className="lg:col-span-7 bg-[#e0f7fb] dark:bg-[#132234] border border-[#cbd5e1] dark:border-[#1e293b] rounded-xl p-5 shadow-lg backdrop-blur-md flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="flex items-center justify-between pb-3 border-b border-[#cbd5e1] dark:border-[#1e293b]">
              <div>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">Selecione quais apps os alunos poderão utilizar.</p>
              </div>
              <span className="text-xs font-bold text-[#0e7c93] dark:text-cyan-300 bg-cyan-100 dark:bg-cyan-950 border border-cyan-200 dark:border-cyan-800/60 px-2.5 py-1 rounded-full">
                  ...
                  <div
                    ...
                    className={"p-3 rounded-xl border transition-all cursor-pointer flex items-start gap-3 select-none " + (
                      isSelected 
                        ...
                        : "border-[#cbd5e1] dark:border-zinc-800 hover:border-cyan-500/50 dark:hover:border-zinc-700 bg-white/60 dark:bg-zinc-950/40 hover:bg-cyan-50/50 dark:hover:bg-zinc-800/40"
                    )}
                  >
                    <div className={"w-5 h-5 rounded mt-0.5 flex items-center justify-center border transition-colors " + (
                      isSelected 
                        ? "bg-[#0e7c93] dark:bg-cyan-500 border-[#0e7c93] dark:border-cyan-500 text-white dark:text-slate-950" 
                        : "border-[#cbd5e1] dark:border-zinc-700 bg-white dark:bg-zinc-900"
                    )}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        {isSelected && (
                          <span className="text-[10px] font-bold text-[#0e7c93] dark:text-cyan-300 bg-cyan-100 dark:bg-cyan-950 px-1.5 py-0.2 rounded border border-cyan-200 dark:border-cyan-800/60">
                            Ativo
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] text-slate-600 dark:text-zinc-400 truncate mt-0.5">{app.subtitulo}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="mt-6 pt-4 border-t border-[#cbd5e1] dark:border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between text-xs gap-3">
            <p className="text-slate-600 dark:text-zinc-400 text-[11px] flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#0e7c93] dark:bg-cyan-400" />
