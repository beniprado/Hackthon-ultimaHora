...
export default function NovaAulaPage() {
  ...
  const { turmas, aplicativos, addAula } = useApp();

  // Form State
  const [selectedTurmaId, setSelectedTurmaId] = useState("turma-1");
  const [selectedDisciplina, setSelectedDisciplina] = useState("Matemática Aplicada");
  const [selectedSala, setSelectedSala] = useState("Sala 101 - Bloco B");
  const [dataAula, setDataAula] = useState("2026-09-15");
  const [horaAula, setHoraAula] = useState("08:30");
  const [horaFim, setHoraFim] = useState("09:20");

  // Whitelist Apps
  const [selectedApps, setSelectedApps] = useState<string[]>(["app-geogebra", "app-docs"]);
