...
export function ActiveScreen({ data, onOpenApp, onFinish }) {
    ...
    <div className="flex-1 min-h-0 flex flex-col bg-white overflow-hidden">
      <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar">
        <div className="px-6 pt-5 pb-5 text-center">
          <div className="w-20 h-20 mx-auto rounded-full bg-cyan-50 flex items-center justify-center mb-3">
            <div className="relative w-14 h-14 rounded-full bg-white border-2 border-[#0e7c93] flex items-center justify-center">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#0e7c93" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect x="3" y="11" width="18" height="11" rx="2" />
                <path d="M7 11V7a5 5 0 0 1 10 0v4" />
              </svg>
              <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-[#0e7c93] flex items-center justify-center text-white">
          ...
          <div className="mt-4 bg-zinc-50 rounded-xl p-3.5 text-left">
            <div className="flex items-start gap-2">
              <span className="w-2 h-2 rounded-full bg-[#0e7c93] mt-1.5 shrink-0" />
              <p className="text-[13px] text-zinc-700 leading-snug">
                <span className="font-semibold">Sessão Ativa:</span> {aula?.sala || "Sala 101"} • {aula?.disciplina || "Matemática Aplicada"} • {aula?.professor || "Prof. Ricardo Silva"}
              </p>
            </div>
          </div>
        </div>
